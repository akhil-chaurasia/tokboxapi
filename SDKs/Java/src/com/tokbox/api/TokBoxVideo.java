...
public class TokBoxVideo {
	...
	public static TokBoxVMail getVmail(TokBoxAPI apiObj, String contentId) {
		try {
			Document document = TokBoxUtils.setupDocument(apiObj.getMessagesWithContent(contentId));

			Node vmailsNode = TokBoxUtils.parseXML("vmailSet", document.getElementsByTagName("vmailSet"));
			Node vmailItem = TokBoxUtils.parseXML("videoMail", vmailsNode.getChildNodes());

			return createVmail(vmailItem);
	...
	private static TokBoxVMail createVmail(Node videomailNode) {
		...
		for (int j = 0; j < contentAttributesLength; j++) {
			Node attr = contentAttributes.item(j);
			...
			if (TokBoxUser.isValid(apiObj, API_Config.PARTNER_SECRET)) {
				...
				for (int i = 0; i < itemNodesLength; i++) {
					...
					if (itemNode.getNodeName().equals("item")) {
						...
						TokBoxVMail newItem = createVmail(videomailNode);
